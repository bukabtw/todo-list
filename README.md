...
- **vue-i18n 9** - международная локализация (i18n)
...
Приложение поддерживает переключение между русским и английским языками.

### Как работает

- Язык определяется автоматически:
  1. Сначала проверяется сохранённый выбор в `localStorage` (ключ `app-locale`)
  2. Если не найден - берётся язык браузера
  3. По умолчанию - русский (`ru`)

- Переключатель языка находится в шапке приложения справа - кнопка с иконкой глобуса и текущим языком (RU/EN)
- При переключении язык сохраняется в `localStorage`
...
- Даты форматируются через `$d()` с учётом локали
- Числа форматируются через `$n()` с учётом локали (валюта: RUB для RU, USD для EN)

### Добавление нового языка

1. Создайте файл `src/locales/{code}.json`
2. Добавьте конфигурацию в `src/i18n.js`
3. Обновите `getDefaultLocale()` для поддержки нового языка
